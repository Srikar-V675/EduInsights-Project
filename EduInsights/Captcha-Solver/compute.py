...
def grades(students_marks):
    ...
    for i in tqdm(range(3, len(students_marks.columns) - 2, 4)):
        result = students_marks.columns[
            i
        ]  # Get the column representing the result (e.g., Internal Marks)
        total = students_marks.columns[
            i - 1
        ]  # Get the column representing the total marks
